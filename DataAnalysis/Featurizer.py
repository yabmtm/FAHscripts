...
class Featurizer(object):
    ...
    def find_traj(self, run, clone, gen):
        ...
        Returns:
            xtc_file: The path of the xtc file. (None: If the file with the given parameters does not exist)
        """

        xtc_filename = os.path.join(self.base_data_dir, 'PROJ%d/RUN%d/CLONE%d/results%d/traj_comp.xtc'%(self.proj_num, run, clone, gen))
	tpr_filename = os.path.join(self.base_data_dir, 'PROJ%d/RUN%d/CLONE%d/frame0.tpr'%(self.proj_num, run, clone, gen))

	if not os.path.isfile(tpr_filename):
            tpr_filename = None

	if os.path.isfile(xtc_filename):
	    return xtc_filename, tpr_filename
	else:
	    print('Error: xtc filename', xtc_filename, 'not found ...')
	    return None, tpr_filename  # return None if there is no traj_comp.xtc


    def rm_periodic_boundary_cond(self, xtc_filename, tpr_filename):
	""" 
	...
	INPUT:
		xtc_filename: the filename of the xtc to process
		tpr_filename: the filename of the *.tpr file needed for gmx trjconv

	Returns:
		xtc:	a tempfile.NamedTemporaryFile() file handle for the processed
			trajectory.  xtc.name is the filename.  This file can be read and written to until
			xtc.close() is called; then it's deleted.
			
	WARNING:  This method assumes there is a frame0.tpr file for doing the conversion.  
		This will NOT be the case for the OpenMM derived class and will have to be
		dealt with then! -VAV
	"""

	import tempfile
	xtc = tempfile.NamedTemporaryFile(suffix='.xtc')
	print('Removing periodic boundary artifacts in temporary file:', xtc.name, '...')

	index_filename = self.gmx_index_filenames[self.run]
	index_group = self.gmx_index_groups[self.run]

	cmd = 'echo "%d\n%d\n" | '%(index_group, index_group)
	cmd += 'gmx trjconv -f %s -s %s -n %s -pbc mol -center -o %s &>/dev/null'%(xtc_filename, tpr_filename, index_filename, xtc.name)
	os.system(cmd)
    ...
    def find_gro(self, gro=None):
        ...
        Returns:
            gro_file: Directory of the .gro file. (None  if no compatible .gro file was found)
        """

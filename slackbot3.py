#!/home/server/packages/anaconda3/bin/python
...
hostname = subprocess.check_output('hostname', shell=True).decode("utf-8").split('.')[0]
...
def get_data_usage():
    ...
    if hostname in ['vav3','vav4']:
        array0_cmd = subprocess.check_output('df -h | grep array0', shell=True).decode("utf-8")
        array1_cmd = subprocess.check_output('df -h | grep array1', shell=True).decode("utf-8")
        for line in [header, array0_cmd, array1_cmd]:
            response += line + '\n'
    elif hostname in ['vav15','vav16']:
        array_cmd = subprocess.check_output('df -h | grep data', shell=True).decode("utf-8")
        for line in [header, array_cmd]:
            response += line + '\n'
    return response
